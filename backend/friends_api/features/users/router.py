...
from friends_api.features.auth.schemas import ChangePasswordRequest, TokenPair
from friends_api.features.users import service
from friends_api.features.users.schemas import Me, UpdateMeRequest
...
@router.put("")
def update_me(body: UpdateMeRequest, db: DbSession, user: CurrentUser) -> Me:
...
def change_password(
    body: ChangePasswordRequest, db: DbSession, auth: Auth, principal: CurrentPrincipal
) -> TokenPair:
